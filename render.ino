// ================================================================
//  render.ino — Screen Rendering (OS MODIFIED)
// ================================================================

// [OS CONCEPT]: CPU BURST TRACKER
// Ekhane render koto microseconds CPU use korche sheta save hobe.
volatile uint32_t renderBurstTimeUs = 0; 

void taskRender() {
  // [OS ADDITION]: Record start time
  ...
  // Actual screen push (I2C communication)
  dispTop.display();
  dispBot.display();

  // [OS ADDITION]: Calculate end time and burst duration
  uint32_t burstEnd = micros();
  renderBurstTimeUs = burstEnd - burstStart; 

  // [DEBUG]: Tumi jodi monitor-e dekhte chao, tahole nicher line-ta uncomment korte paro.
  // Serial.print("R_BURST: "); Serial.println(renderBurstTimeUs);
}

// ──────────────────────────────────────────────────────────────────
//  Baki shob render logic (showSplash, renderHome, etc.) thakbe niche
// ──────────────────────────────────────────────────────────────────
...
void renderPlaying() {
  ...
  for (uint8_t i = 0; i < MAX_BULLETS; i++) {
    ...
    vDrawFastVLine((int16_t)playerBullets[i].x, (int16_t)playerBullets[i].y, BULLET_H, SSD1306_WHITE);
  }
  ...
  vFillTriangle(sx, SHIP_Y - 5, sx - SHIP_HALF_W, SHIP_Y, sx + SHIP_HALF_W, SHIP_Y, SSD1306_WHITE);
...
void renderSettings() {
  ...
  vDrawFastHLine(0, 14, VIRTUAL_W, SSD1306_WHITE);
  char bBuf[14];
  snprintf(bBuf, sizeof(bBuf), "%sBRIGHT:%d/3", settingsCursor==SETTINGS_BRIGHTNESS ? ">" : " ", brightnessIdx);
  vPrintStr(4, 24, bBuf, 1);
  char sBuf[14];
  snprintf(sBuf, sizeof(sBuf), "%sBTN:%s", settingsCursor==SETTINGS_BTNSWAP ? ">" : " ", btnSwapped ? "SWAP" : "NORM");
